...
export const leads = pgTable("leads", {
  ...
  email: text("email"),
  telefone: text("telefone"),
  status: text("status").notNull().default("novo"), // novo, em andamento, convertido, perdido
  origem: text("origem"), // fonte do lead
...
export const contracts = pgTable("contracts", {
  id: serial("id").primaryKey(),
  oportunidadeId: integer("oportunidade_id").references(() => opportunities.id),
  clienteId: integer("cliente_id").references(() => customers.id),
  valor: decimal("valor", { precision: 10, scale: 2 }).notNull(),
  dataInicio: timestamp("data_inicio").notNull(),
  dataFim: timestamp("data_fim"),
  status: text("status").notNull().default("ativo"), // ativo, cancelado, expirado
  criadoEm: timestamp("criado_em").defaultNow(),
});
